...
<div class="verification-card">
    ...
    <div class="info-row">
        ...
        <div class="info-value">{{ $pembayaran->user->name }}</div>
    </div>
    <div class="info-row">
        <div class="info-label">Bulan/Tahun:</div>
        <div class="info-value">{{ $pembayaran->bulan_tahun }}</div>
    </div>
    <div class="info-row">
    ...
    <div style="margin: 20px 0;">
        ...
        @if($pembayaran->bukti_pembayaran)
            <img src="{{ asset('storage/' . $pembayaran->bukti_pembayaran) }}" class="bukti-img" style="margin-top: 10px; max-height: 300px; object-fit: contain;">
        @else
            <p style="color: #64748b;">Tidak ada bukti pembayaran</p>
        @endif
    </div>

    <form action="{{ route('pembayaran.confirm', $pembayaran->id) }}" method="POST">
        @csrf
        
        <div style="display: flex; gap: 15px; margin: 20px 0; flex-wrap: wrap;">
            <button type="submit" name="status" value="diterima" class="btn-terima" onclick="return confirm('Terima pembayaran ini?')">
                <i class="fas fa-check-circle"></i> Terima & Lunaskan
            </button>
            <button type="submit" name="status" value="ditolak" class="btn-tolak" onclick="return confirm('Tolak pembayaran ini?')">
    ...
    <div style="margin-top: 20px;">
        <a href="{{ route('pembayaran.index') }}" class="btn-cancel">Kembali</a>
    </div>
</div>

<script>
    document.querySelector('button[value="ditolak"]').addEventListener('click', function(e) {
